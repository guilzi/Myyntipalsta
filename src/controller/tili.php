...
function lisaaTili($formdata) {
  ...
  // Tarkistetaan onko puhelinnumero määritelty ja että
  // puhelinnumerossa on vain numeroita tai '+' '-' merkit.
  if (!isset($formdata['puhnro']) || !$formdata['puhnro']) {
    $error['phone'] = "Anna puhelinnumerosi.";
} else {
    if (!preg_match("/^[0-9+\-()\s]+$/", $formdata["puhnro"])) {
        $error['puhnro'] = "Syötä puhelinnumerosi ilman erikoismerkkejä, paitsi '+' tai '-'.";
    }
  ...
  if (!$error) {
    ...
    $idhenkilo = lisaaHenkilo($nimi,$puhnro,$email,$discord,$salasana);
